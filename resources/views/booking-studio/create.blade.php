...
<div class="p-6 max-w-4xl mx-auto">
    <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h1 class="text-2xl font-bold text-slate-800">Tambah Booking Studio</h1>
        <p class="mt-1 text-sm text-slate-500">Pilih pelanggan, studio, tanggal, dan jam. Total dihitung otomatis dari harga studio.</p>
    </div>

    <form action="{{ route('booking-studio.store') }}" method="POST" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
            ...
            <div>
                ...
                <select id="studioSelect" name="studio_id" class="w-full rounded-xl border border-slate-300 p-3" required>
                    ...
                    @foreach($studio as $item)
                        <option value="{{ $item->id }}" data-price="{{ $item->harga_per_jam }}">{{ $item->nama_studio }} (Rp {{ number_format($item->harga_per_jam, 0, ',', '.') }}/jam)</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Tanggal Booking</label>
                <input type="date" name="tanggal_booking" class="w-full rounded-xl border border-slate-300 p-3" required>
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Jam Mulai</label>
                <input type="time" id="jamMulai" name="jam_mulai" class="w-full rounded-xl border border-slate-300 p-3" required>
            </div>
            <div>
                <label class="mb-2 block text-sm font-semibold text-slate-700">Jam Selesai</label>
                <input type="time" id="jamSelesai" name="jam_selesai" class="w-full rounded-xl border border-slate-300 p-3" required>
            </div>
            <div>
        ...
        <div class="mt-6 rounded-xl border border-blue-100 bg-blue-50 p-4">
            <div class="flex items-center justify-between">
                ...
                <div class="text-right">
                    <p id="totalPreview" class="text-xl font-bold text-blue-800">Rp 0</p>
                </div>
            </div>
        </div>

        <div class="mt-6 flex justify-end">
            <button type="submit" class="rounded-xl bg-blue-600 px-5 py-2.5 font-semibold text-white transition hover:bg-blue-700">Simpan</button>
        </div>
    </form>
</div>
